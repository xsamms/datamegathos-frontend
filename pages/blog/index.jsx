...
export async function getStaticProps() {
  const res = await fetch(
    `${API_URI}/api/posts?sort=date:DESC&pagination[limit]=10&populate=*`
  );
  ...
  const feat = await fetch(
    `${API_URI}/api/posts?filters[featured_post][$eq]=true&sort=date:DESC&pagination[limit]=5&populate=*`
  );
  ...
  return {
    props: { posts, category, featured, tags },
    revalidate: 1,
  };
}
