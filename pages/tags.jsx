            ...
            <div className="col-lg-4">
              <div className="sidebar-widget-area mb-40">
                ...
                <div className="widget tags_cloud_widget mb-40 wow fadeInUp">
                  ...
                  {taags.data.map((t) => {
                    return (
                      <Link key={t.id} href={`/tags?tag=${t.attributes.slug}`}>
                        {t.attributes.Name}
                      </Link>
                    );
                  })}
...
export async function getServerSideProps({ query: { tag } }) {
  const res = await fetch(
    `${API_URI}/api/posts?filters[tags][slug][$contains]=${tag}&sort=date:DESC&populate=*`
  );
  ...
  const taag = await fetch(`${API_URI}/api/tags`);
  const taags = await taag.json();

  return {
    props: { tg, categories, featured, taags },
  };
}
